...
class Conv(object):
  ...
  def forward(x, w, b, conv_param):
    ...
    afterpad = torch.nn.functional.pad( x,(conv_param['pad'],conv_param['pad'],conv_param['pad'],conv_param['pad']),'constant',0.0 )
    N,C,H,W = x.shape
    F,C,HH,WW = w.shape
    newH = 1 + (H + 2 * conv_param['pad'] - HH) / conv_param['stride']
    newH = int(newH)
    newW = 1 + (H + 2 * conv_param['pad'] - WW) / conv_param['stride']
    newW = int(newW)
    out = torch.zeros(N,F,newH,newW,dtype=x.dtype,device=x.device)
    for n in range(N):
      for f in range(F):
        outi = 0 
        for i in range(0,afterpad.shape[2] - HH + 1,conv_param['stride']):
          outj =0
          for j in range(0,afterpad.shape[3] - WW + 1,conv_param['stride']):
            tmp = 0
            for c in range(C):
              for hh in range(HH):
                for ww in range(WW):
                  tmp += afterpad[n][c][i+hh][j+ww] * w[f][c][hh][ww] 
                  
            out[n][f][outi][outj] = tmp + b[f]
            outj += 1
          outi+=1
      ...
      for f in range(F):
        for i in range(Hout):
          for j in range(Wout):
            ...
            dw[f] += dout[n,f,i,j] * afterpad[n, :, i * stride: i * stride +  HH, j * stride : j * stride + WW]
            db[f] += dout[n,f,i,j]
...
class MaxPool(object):
  ...
  def forward(x, pool_param):
    ...
    newH = int( 1 + (H - pool_height) / pool_stride )
    newW = int( 1 + (W - pool_width) / pool_stride )
    out = torch.zeros(N,C,newH,newW,dtype=x.dtype,device=x.device)

    for n in range(N):
      for c in range(C):
        for h in range(newH):
          for w in range(newW):
            out[n,c,h,w] = torch.max(x[n,c,h*pool_stride:h*pool_stride + pool_height,w*pool_stride : w*pool_stride + pool_width])
        ...
        for c in range(C):
            for i in range(new_H):
                for j in range(new_W):
                    ...
                    dx[n,c,i*pool_stride:i*pool_stride + pool_height,j*pool_stride:j*pool_stride + pool_width] = (window == torch.max(window)) * dout[n,c,i,j]
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    return dx
